...
describe('CriteoAnalyticsAdapter', function () {
  ...
  it('send events to endpoint after auction end event', function (done) {
    ...
    setTimeout(() => {
      ...
      expect(requests[0].url).to.equal('//bidder.criteo.com/prebid/analytics');
      expect(requests[0].method).to.equal('POST');
      const requestBody = JSON.parse(requests[0].requestBody);
      expect(requestBody.auctionEnd).to.deep.equal([ { aField: 'aFieldValue', durationSinceAuctionStart: 2 } ]);
      expect(requestBody.bidTimeout).to.deep.equal([ { aTimeoutField: 'aTimeoutFieldValue', durationSinceAuctionStart: 2 } ]);
      done();
    }, 0)
  });
  ...
  it('adds duration relative to auction init timestamp on events', function (done) {
    ...
    setTimeout(() => {
      expect(requests.length).to.equal(1);
      const requestBody = JSON.parse(requests[0].requestBody);
      expect(requestBody.auctionInit).to.deep.equal([ { timestamp: 1, config: { sendDelay: 0 }, durationSinceAuctionStart: 1 } ]);
      expect(requestBody.auctionEnd).to.deep.equal([ { aField: 'aFieldValue', durationSinceAuctionStart: 1 } ]);
      done();
    }, 0)
  });

  it('send performance entries events', function (done) {
    sinon.stub(window.performance, 'getEntries').returns([ { duration: 1, name: 'http://bidder.criteo.com/cdb' } ]);
    events.emit(constants.EVENTS.AUCTION_END, { aField: 'aFieldValue' });
    ...
    setTimeout(() => {
      expect(requests.length).to.equal(1);
      const requestBody = JSON.parse(requests[0].requestBody);
      expect(requestBody.performanceEntries).to.deep.equal({ criteo: [ { duration: 1, name: 'http://bidder.criteo.com/cdb' } ] });
